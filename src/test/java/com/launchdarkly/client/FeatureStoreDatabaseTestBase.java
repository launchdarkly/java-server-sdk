...
public abstract class FeatureStoreDatabaseTestBase<T extends FeatureStore> extends FeatureStoreTestBase<T> {
  ...
  public void storesWithDifferentPrefixAreIndependent() throws Exception {
    ...
    try {
      ...
      assertEquals(flag2a.getVersion(), store1.get(FEATURES, flag2a.getKey()).getVersion());
      assertEquals(flag2c.getVersion(), store1.get(FEATURES, flag2c.getKey()).getVersion());
    } finally {
      store1.close();
      store2.close();
    }
